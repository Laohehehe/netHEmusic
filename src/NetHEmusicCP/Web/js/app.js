...
function applyPerfAnim(s) {
  ...
  // 品牌标（GitHub / Gitee）是实心填充图形，不能用描边那套
  function icoFill(inner) {
    return '<svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor" stroke="none">' + inner + '</svg>';
  }
  var SVG = {
  ...
  async function goSettings() {
    ...
    function repoRow(s) {
      var r = el('div','set-row');
      r.appendChild(el('label','','开源仓库'));
      var box = el('div','repo-btns');
      [['github', SVG.github, 'GitHub', s.repoGithub], ['gitee', SVG.gitee, 'Gitee', s.repoGitee]].forEach(function (it) {
        var b = el('button','repo-btn', it[1]);
        b.type = 'button';
        b.title = it[2] + ' 仓库：' + (it[3] || '');
        b.onclick = function () { NE.post({ type: 'open_repo', name: it[0] }); };
        box.appendChild(b);
      });
      r.appendChild(box);
      return r;
